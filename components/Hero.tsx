...
const Hero: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      .fromTo(marqueeWrapperRef.current,
  ...
  return (
    <section id="hero" ref={containerRef} className="relative pt-32 pb-20 md:pt-48 md:pb-32 px-6 overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        <h1 ref={titleRef} className="text-4xl md:text-6xl lg:text-7xl font-bold leading-[0.95] tracking-tight text-neutral-900 mb-6 text-balance">
          We Design AI-Powered Websites <br/>
          That Turn <span className="text-neutral-400 font-light italic">Vision</span> Into Revenue
        </h1>
        
        {/* Infinite Scrolling Marquee */}
        <div ref={marqueeWrapperRef} className="relative w-full max-w-6xl mx-auto my-12 md:my-16 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
            <div ref={marqueeTrackRef} className="flex gap-4 md:gap-8 w-max pl-4">
                {marqueeIcons.map((url, i) => (
                    <div key={i} className="w-16 h-16 md:w-20 md:h-20 bg-white border border-neutral-200 shadow-sm rounded-2xl flex items-center justify-center shrink-0 p-4 hover:border-neutral-300 transition-colors">
                        <img 
                          src={url} 
                          alt="Tech Logo" 
        ...
        <div ref={ctaRef} className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="#contact" className="px-8 py-4 bg-neutral-900 text-white rounded-xl font-semibold text-lg hover:bg-black transition-all hover:-translate-y-1 shadow-lg hover:shadow-xl min-w-[180px] flex items-center justify-center gap-2">
                Contact Us
            </a>
            <a href="#work" className="px-8 py-4 bg-white text-neutral-900 border border-neutral-200 rounded-xl font-semibold text-lg hover:bg-neutral-50 transition-all hover:-translate-y-1 shadow-sm hover:shadow-md min-w-[180px] flex items-center justify-center gap-2 group">
                Explore Our Work <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform"/>
            </a>
        </div>

        {/* Supporting Line */}
        <p ref={supportRef} className="mt-10 text-sm font-medium text-neutral-500">
