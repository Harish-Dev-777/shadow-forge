    ...
    <div ref={containerRef} className="bg-white min-h-screen">
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <div
              className={`inline-block px-4 py-1.5 rounded-full ${service.iconBg} ${service.iconColor} text-sm font-bold mb-6 anim-text`}
            >
              {service.title}
            </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {service.fullContent.features.map((feature, i) => (
              <div
                key={i}
                className="flex items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-neutral-100"
              >
